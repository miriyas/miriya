  ...
  proto.columnPack = function (rect) {
    for (var i = 0; i < this.spaces.length; i++) {
      ...
      var canFitInSpaceColumn =
        space.x <= rect.x && space.x + space.width >= rect.x + rect.width && space.height >= rect.height - 0.01; // fudge number for rounding error
  ...
  proto.rowPack = function (rect) {
    for (var i = 0; i < this.spaces.length; i++) {
      ...
      var canFitInSpaceRow =
        space.y <= rect.y && space.y + space.height >= rect.y + rect.height && space.width >= rect.width - 0.01; // fudge number for rounding error
      if (canFitInSpaceRow) {
        rect.x = space.x;
        this.placed(rect);
        break;
...
(function (window, factory) {
  ...
  if (typeof define == 'function' && define.amd) {
    // AMD
    define(['../layout-mode', 'packery/js/packery'], factory);
  } else if (typeof module == 'object' && module.exports) {
    // CommonJS
    module.exports = factory(require('../layout-mode'), require('packery'));
  } else {
    // browser global
